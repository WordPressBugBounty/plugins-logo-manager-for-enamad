...
function enamad_update_admin_message() {
    ...
    $settings_url = menu_page_url( 'enamadlogo-options', false );
    ...
    $dismiss_url = add_query_arg(
            'update_enamad_new_ver_notice_applied_0_2',
            '1',
            $settings_url
    );
    ...
    $message = sprintf(
    /* translators: %1$s: opening link tag, %2$s: closing link tag, %3$s: dismiss link */
            __(
                    'نسخه جدید نماد الکترونیکی دچار تغییراتی شده است. لطفا جهت تنظیمات به %1$sاینجا%2$s رفته و کد جدید را در قسمت مربوطه وارد نمایید. %3$s',
            ...
            sprintf(
                    '<a href="%s">× %s</a>',
                    esc_url( $dismiss_url ),
                    esc_html__( 'حذف این پیام', 'logo-manager-for-enamad' )
            )
    );
